 ...
 */
Event.prototype.on = function on(eventName, handle) { // eslint-disable-line
 ...
 * @param {function} handle 事件监听函数
 */
Event.prototype.once = function once(eventName, handle) { // eslint-disable-line
  eventNameCheck(eventName);
  handleCheck(handle);
  const that = this;
 ...
 */
Event.prototype.emit = function emit(eventName, data) { // eslint-disable-line
  eventNameCheck(eventName);
  // 如果有eventName事件的兼听函数
  if (this.eventNames[eventName]) {
 ...
 * @returns 
 */
Event.prototype.remove = function remove(eventName, handle) { // eslint-disable-line
  eventNameCheck(eventName);
  handleCheck(handle);
  const oldEventName = this.eventNames[eventName];
  if (oldEventName) {
 ...
 */
Event.prototype.removeAll = function removeAll(eventName) { // eslint-disable-line
  if (eventName) {
    eventNameCheck(eventName);
    const oldEventName = this.eventNames[eventName];
    if (oldEventName) {
